...
Original file is located at
    https://colab.research.google.com/drive/1xniLWTSqz496TEfj36Cp35cwRpBtv98O
...
def color_jitter(image, c_factor=None, b_factor=None, s_factor=None):
    ...
    if c_factor is not None:
      return cv2.convertScaleAbs(image, alpha=c_factor, beta=0)
    if b_factor is not None:
      return cv2.convertScaleAbs(image, alpha=1, beta=b_factor)
...
def get_cnn_model():
    ...
    # Block 3
    model.add(layers.Flatten()) # 替代 GlobalAveragePooling2D
...
if __name__ == '__main__':
    ...
    history = auto_encoder.fit(x=X_train, y=y_train, epochs=100, batch_size=32, validation_data=(X_test, y_test))
